...
int main(int argc, char *argv[])
{
	...
	desc.add_options()
		...
		("esfile,e", po::value<std::string>(&esFchkName)->required(), "the excited state Fchk file")
	...
	std::cout << "Threshold for including 3rd/4th derivatives: " << deriv_thres << std::endl;
	 ...
	 * need to be equal
	 */
	Eigen::VectorXi nBasis = Eigen::VectorXi::Zero(Nmodes);
	Eigen::VectorXd lowBound = Eigen::VectorXd::Zero(Nmodes);
	Eigen::VectorXd uppBound = Eigen::VectorXd::Zero(Nmodes);
	...
	for (int i = 0; i < Nmodes; i++)
		IVRinput << "    q_" << std::setfill('0') << std::setw(3) << i + 1
				 << "  ho  " << std::setw(3) << std::setfill(' ') << int(nBasis(i)) << "  xi-xf  "
				 << std::fixed << std::setfill(' ') << std::setw(8)
				 << double(lowBound(i))
				 << std::fixed << std::setfill(' ') << std::setw(8)
				 << double(uppBound(i)) << std::endl;
